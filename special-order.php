...
$barcodeCmd = "/usr/local/bin/barcode -e ean13 -n -b \"$orderInfo[ean13]\" -D -o results/$filename-13.txt -g100x30+160+135";
$stampCmd = "$pdftk $pdf_name.pre stamp results/$filename-13.pdf output $pdf_name";

exec( $xfdfCmd, $outputXfdf, $retXfdf );
exec( $barcodeCmd, $outputBarcode, $retBarcode );
exec( "cat barcode_doc_front.txt results/$filename-13.txt barcode_doc_back.txt > results/$filename-13.pdf", $outputCat, $retCat);
exec( $stampCmd, $outputStamp, $retStamp );

if ( file_exists( './results/' . $filename . '-13.pdf' ) ) {

	unlink( './results/' . $filename . '-13.pdf' );
}

if ( file_exists( './results/' . $filename . '-13.txt' ) ) {
	unlink( './results/' . $filename . '-13.txt' );
}

if ( file_exists( './results/' . $filename . '.pdf.pre' ) ) {
	unlink( './results/' . $filename . '.pdf.pre' );
}

// Print the final PDF
system( "/usr/bin/lpr -o landscape -P Brother_HL-5470DW results/$pdf_file_name" ); 
